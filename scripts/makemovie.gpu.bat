...
if exist "ffmpeg.exe" (
    ffmpeg -framerate 60 -strict 2 -hwaccel auto -i capture/%%4d.jpg -i capture/audio.wav  -c:v hevc_nvenc -c:a aac -rc vbr -cq 24 -qmin 24 -qmax 24 -profile:v main10 -pix_fmt p010le -b:v 4M -c:a aac -map 0 "%timestamp%.mp4"
) else (
	@echo on
