...
if exist "ffmpeg.exe" (
    ffmpeg.exe -framerate 60 -hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 2 -i capture/%%4d.jpg -i capture/audio.wav -c:v h264_nvenc -c:a aac -rc vbr -cq 24 -qmin 24 -qmax 24 -b:v 10M -preset slow %timestamp%.mp4
) else (
	@echo on
